...
testRestoreUsersMissingPathArgument() {
  $cmd -- -m restoreUsers >/dev/null 2>&1
  ret_code=$?
  assertEquals "${RET_ARGS_VALIDATION}" "${ret_code}"
}

testRestoreUsersMissingMethodArgument() {
...
testRestoreUsersIncorrectPathArgument() {
  config_file="resources/mocks/fake.json"
  $cmd -- -p "${config_file}" -m restoreUsers >/dev/null 2>&1
  ret_code=$?
  assertEquals "${RET_ARGS_VALIDATION}" "${ret_code}"
}

testRestoreUsersIncorrectMethodArgument() {
  config_file="resources/mocks/success/user/valid_user.json"
  $cmd -- -p "${config_file}" -m restoreUsersFake >/dev/null 2>&1
  ret_code=$?
  assertEquals "${RET_ARGS_VALIDATION}" "${ret_code}"
}

testRestoreUsersMissingConfigData() {
...
testRestoreUsersMissingBocaUrl() {
...
testRestoreUsersMissingResultFilePath() {
...
testRestoreUsersInvalidBocaUrl() {
...
testRestoreUsersInvalidResultFilePath() {
...
testRestoreUsersIncorrectBocaUrl() {
...
testRestoreUsersIncorrectResultFilePath() {
...
testRestoreUsersMissingLoginData() {
...
testRestoreUsersMissingUsername() {
...
testRestoreUsersMissingPassword() {
...
testRestoreUsersInvalidUsername() {
...
testRestoreUsersInvalidPassword() {
...
testRestoreUsersIncorrectUsername() {
...
testRestoreUsersIncorrectPassword() {
...
testRestoreUsersMissingUserData() {
  config_file="resources/mocks/fail/user/missing_user.json"
  testRestoreValidUsers "${config_file}"
}

testRestoreValidUsers() {
  if [ -n "$1" ]; then
    config_file="$1"
  else
    config_file="resources/mocks/success/user/valid_user.json"
