...
testRestoreProblemsMissingPathArgument() {
  $cmd -- -m restoreProblems >/dev/null 2>&1
  ret_code=$?
  assertEquals "${RET_ARGS_VALIDATION}" "${ret_code}"
}

testRestoreProblemsMissingMethodArgument() {
...
testRestoreProblemsIncorrectPathArgument() {
  config_file="resources/mocks/fake.json"
  $cmd -- -p "${config_file}" -m restoreProblems >/dev/null 2>&1
  ret_code=$?
  assertEquals "${RET_ARGS_VALIDATION}" "${ret_code}"
}

testRestoreProblemsIncorrectMethodArgument() {
  config_file="resources/mocks/success/problem/valid_problem.json"
  $cmd -- -p "${config_file}" -m restoreProblemsFake >/dev/null 2>&1
  ret_code=$?
  assertEquals "${RET_ARGS_VALIDATION}" "${ret_code}"
}

testRestoreProblemsMissingConfigData() {
...
testRestoreProblemsMissingBocaUrl() {
...
testRestoreProblemsMissingResultFilePath() {
...
testRestoreProblemsInvalidBocaUrl() {
...
testRestoreProblemsInvalidResultFilePath() {
...
testRestoreProblemsIncorrectBocaUrl() {
...
testRestoreProblemsIncorrectResultFilePath() {
...
testRestoreProblemsMissingLoginData() {
...
testRestoreProblemsMissingUsername() {
...
testRestoreProblemsMissingPassword() {
...
testRestoreProblemsInvalidUsername() {
...
testRestoreProblemsInvalidPassword() {
...
testRestoreProblemsIncorrectUsername() {
...
testRestoreProblemsIncorrectPassword() {
...
testRestoreProblemsMissingProblemData() {
  config_file="resources/mocks/fail/problem/missing_problem.json"
  testRestoreValidProblems "${config_file}"
}

testRestoreValidProblems() {
  if [ -n "$1" ]; then
    config_file="$1"
  else
    config_file="resources/mocks/success/problem/valid_problem.json"
