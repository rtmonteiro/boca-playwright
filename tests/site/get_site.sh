...
testGetSiteMissingPathArgument() {
  $cmd -- -m getSite >/dev/null 2>&1
  ret_code=$?
  assertEquals "${RET_ARGS_VALIDATION}" "${ret_code}"
}

testGetSiteMissingMethodArgument() {
  config_file="resources/mocks/success/site/valid_site.json"
...
testGetSiteIncorrectPathArgument() {
...
testGetSiteIncorrectMethodArgument() {
  config_file="resources/mocks/success/site/valid_site.json"
  $cmd -- -p "${config_file}" -m getSiteFake >/dev/null 2>&1
  ret_code=$?
  assertEquals "${RET_ARGS_VALIDATION}" "${ret_code}"
}

testGetSiteMissingConfigData() {
...
testGetSiteMissingResultFilePath() {
...
testGetSiteInvalidResultFilePath() {
...
testGetSiteIncorrectResultFilePath() {
...
testGetValidSite() {
  ...
  # Check if the returned site has the same id of the configuration file
  jsonIn=$(jq -S '.site | .id' "../${config_file}")
