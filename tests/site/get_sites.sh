...
testGetSitesMissingPathArgument() {
  $cmd -- -m getSites >/dev/null 2>&1
  ret_code=$?
  assertEquals "${RET_ARGS_VALIDATION}" "${ret_code}"
}

testGetSitesMissingMethodArgument() {
...
testGetSitesIncorrectPathArgument() {
  config_file="resources/mocks/fake.json"
  $cmd -- -p "${config_file}" -m getSites >/dev/null 2>&1
  ret_code=$?
  assertEquals "${RET_ARGS_VALIDATION}" "${ret_code}"
}

testGetSitesIncorrectMethodArgument() {
  config_file="resources/mocks/success/site/valid_site.json"
  $cmd -- -p "${config_file}" -m getSitesFake >/dev/null 2>&1
  ret_code=$?
  assertEquals "${RET_ARGS_VALIDATION}" "${ret_code}"
}

testGetSitesMissingConfigData() {
...
testGetSitesMissingResultFilePath() {
...
testGetSitesInvalidResultFilePath() {
...
testGetSitesIncorrectResultFilePath() {
...
testGetSitesMissingSiteData() {
  config_file="resources/mocks/fail/site/missing_site.json"
  testGetValidSites "${config_file}"
}
