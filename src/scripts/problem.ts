...
async function fillProblems(page: Page, problem: Problem) {
  ...
  await page.locator('input[name="problemnumber"]')
    .fill(problem.id.toString());
  await page.locator('input[name="problemname"]')
    .fill(problem.name);
  await page.locator('input[name="probleminput"]')
    .setInputFiles(problem.filePath);
  if (problem.colorName) {
    await page.locator('input[name="colorname"]')
