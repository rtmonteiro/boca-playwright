...
export async function fillSite(page: Page, site: SiteModel) {
  ...
  if (site.id) {
    await page.locator('select[name="site"]')
      .selectOption(site.id.toString());
  } else {
  ...
  if (site.globalScore) {
    await page.locator('input[name="globalscore"]')
  ...
  if (site.scoreLevel) {
    await page.locator('input[name="scorelevel"]')
      .fill(site.scoreLevel.toString());
  }

  if (site.globalScoreboard) {
    await page.locator('input[name="globalscoreboard"]')
