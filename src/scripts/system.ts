...
export async function createContest (page: Page, contest: ContestModel): Promise<void> {
...
export async function clearContest (page: Page, contest: ContestModel): Promise<void> {
  await page.goto(BASE_URL + '/system/');
  await page.getByRole('link', { name: 'Contest' }).click();
  await page.locator('select[name="contest"]').selectOption(contest.setup.id?.toString() ?? 'new');
